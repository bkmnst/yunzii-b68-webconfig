...
export const B68_COLOR_GROUP_BASE = 0x20
export const B68_COLOR_GROUP_MAX = 7

export const B68_ONBOARD_COLORS = Object.freeze([
  { group: 0, name: 'Red', css: '#ff0000' },
  { group: 1, name: 'Green', css: '#00ff00' },
  { group: 2, name: 'Blue', css: '#0000ff' },
  { group: 3, name: 'Cyan', css: '#00ffff' },
  { group: 4, name: 'Yellow', css: '#ffff00' },
  { group: 5, name: 'Magenta', css: '#ff00ff' },
  { group: 6, name: 'White', css: '#ffffff' },
] as const)
...
export function buildSetConfigurationPayload(
  ...
  if (patch.colorGroup !== undefined
    && (!Number.isInteger(patch.colorGroup) || patch.colorGroup < 0 || patch.colorGroup > B68_COLOR_GROUP_MAX)) {
    throw new RangeError('B68 onboard color group must be an integer from 0 to 7.')
  }
  ...
  if (patch.colorGroup !== undefined) payload[7 + 11] = B68_COLOR_GROUP_BASE + patch.colorGroup
  return payload
}
...
export function parseB68OnboardConfiguration(bytes: ArrayLike<number>): B68OnboardConfiguration {
  ...
  const effectParameter = bytes[11]
  const colorGroup = effectParameter - B68_COLOR_GROUP_BASE
  if (colorGroup < 0 || colorGroup > B68_COLOR_GROUP_MAX) {
    throw new RangeError('B68 onboard color group is outside its confirmed 0–7 range.')
  }
  const colorName = colorGroup === 7 ? 'Random'
    : B68_ONBOARD_COLORS.find((color) => color.group === colorGroup)?.name ?? `Group ${colorGroup}`
  return {
    debounceMs,
    speedLevel,
    brightnessLevel,
