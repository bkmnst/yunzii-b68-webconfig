...
Onboard color is the selected effect's group index encoded at record offset 11 as `StartGp + group`. B68 sets `StartGp=0x20` and `RandomVal=7`. Native initialization proves groups 0–6 use the fixed `COLORREF` palette red, green, blue, cyan, yellow, magenta, and white; group 7 is random. The selector is gated by each `LedOpt` row's random/fixed-color flags, and a typed write changes only offset 11 before requiring matching `GetLED` readback.
